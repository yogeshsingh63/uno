  ...
  return (
    <Animated.View style={[styles.container, glowStyle, popStyle, { shadowColor: colorHex, width: cardWidth, height: cardHeight }]}>
      ...
      <View style={[styles.layer, styles.layer2]}>
        {previousCard ? (
          <CardComponent card={previousCard} disabled size="discard" cardWidth={cardWidth} cardHeight={cardHeight} />
        ) : (
          <CardBack width={cardWidth} height={cardHeight} borderRadius={10} />
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  layer1: {
    opacity: 0.35,
    transform: [{ rotate: '4deg' }, { translateX: 3 }, { translateY: -2 }],
  },
  layer2: {
    opacity: 0.65,
    transform: [{ rotate: '-2.5deg' }, { translateX: -2 }, { translateY: 1 }],
  },
  topCardLayer: {
    zIndex: 3,
  },
